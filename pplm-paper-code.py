...
SmallConst = 1e-15
enc = GPT2Tokenizer.from_pretrained('gpt2-medium')#('gpt-2_pt_models/345M/')
...
def perturb_past(past, model, prev, args, classifier, good_index=None, stepsize=0.01, vocab_size=50257,
    ...
    for i in range(args.num_iterations):
        ...
        if args.loss_type == 2 or args.loss_type == 3:
            ce_loss = torch.nn.CrossEntropyLoss()#use logits to do several querys, calculate mean value ; no rational explation
...
def sample_from_hidden(model, args, classifier, context=None, past=None, device='cuda',
    ...
    for i in trange(args.length, ascii=True):
        ...
        if past is None and output is not None:
            prev = output[:, -1:]
            _, past = model(output[:, :-1]) #As we treated it as query, so we should use the past that does not contains the final one
...
def run_model():
    ...
    for out in seq:
        ...
        out1, out_perturb, discrim_loss_list, loss_in_time_list = latent_perturb(model=model, args=args, context=out,
                                                                 device=device) 
        # each time process 1 sentence, but transformers takes a batch as input, so it will be unsqueeze later
